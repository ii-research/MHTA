...
def evaluate_on_mctaco(model, tokenizer, device, epoch, output_dir, config):
    ...
    self_reflect=config["evaluation"]["self_reflect"]
    ...
    sf_prefix = "You are an AI assistant that uses a Chain of Thought (CoT) approach with reflection to answer queries. Follow these steps: \nStep 1. Reason through the problem step by step within the <reasoning> tags. \nStep 2. Given your previous reasoning, identify relevant temporal events in the given context for answering the given question within <timeline> tags. Assume relations in the context are unidirectional. \nStep 3. Reflect on your reasoning and the timeline to check for any errors or improvements within the <reflection> tags. \nStep 4. Make any necessary adjustments based on your reflection. If there is additional reasoning required, go back to Step 1 (reason through the problem step-by-step), otherwise move to Step 5. \nStep 5. Provide your final, concise answer within the <answer> tags. Answer true or false. Provide the answer at the beginning of the response.  \nImportant: The <reasoning>, <timeline>, and <reflection> sections are for your internal reasoning process. Do not use enumerations or lists; use plain paragraph-style explanations. The final output must be entirely contained within the <answer> tags."



    def make_prompt(passage, question, answer, label):
        if prompt_template == 2:
            return fs_prefix + f"Based on the information presented in the passage: {passage}\nCan the candidate answer \"{answer}\" answer the question \"{question}\"?\n{cot_suffix}The answer is: {label}"
        elif prompt_template == 3:
            return fs_prefix + f"According to the passage: {passage}\nIs the candidate answer \"{answer}\" correct to the question \"{question}\"?\n{cot_suffix}The answer is {label}"
        else:
    ...
    def make_prompt_sf(passage, question, answer):
        if prompt_template == 2:
            return sf_prefix + f'Based on the information presented in the passage "{passage}", can the candidate answer "{answer}" answer the question "{question}"?{cot_suffix} The answer is:'
        elif prompt_template == 3:
            return sf_prefix + f'According to the passage "{passage}", is the candidate answer "{answer}" correct to the question "{question}"?{cot_suffix} The answer is:'
        else:
            return sf_prefix + f" \nIs the following candidate answer to the question true or false according to the passage?\nPassage: {passage}\nQuestion: {question}\nCandidate answer: {answer}"

    model.eval()
    if self_reflect:
        prompts = [make_prompt_sf(p, q, a) for p, q, a in
                   zip(dataset["passage"], dataset["question"], dataset["answer"])]
        for i in tqdm(range(0, len(prompts), batch_size), desc="Evaluating on MC-TACO"):
            ...
            for j, full_text in enumerate(decoded):
                ...
                trimmed = full_text[prompt_len:].strip()
                trimmed_list.append(trimmed)
            for response in trimmed_list:
                predictions.append(1 if any(w in response.lower() for w in ["yes", "true", "correct"]) else 0)
...
def evaluate_on_td(model, tokenizer, device, epoch, output_dir, config):
    ...
    few_shot_examples = [
        ...
        "Dialogue:\nA:I just checked the weather forecast for tomorrow .\nB: Oh ? What does it say ?\nA: It says it will rain heavily in the morning and the wind will be strong .\nB: That is bad news . We were planning to go hiking .\nA: Maybe we can still go if the rain stops early .\nB: True . The forecast says the rain will probably stop by <MASK> .\nA: That is not too bad . We can leave after that .\nB: Yes , and we can still enjoy the afternoon outdoors .\n\nIs it appropriate to fill in the <MASK> with \"noon\" in this dialogue?\nThe answer is: yes"
    ]

    fs_prefix = "\n\n".join(few_shot_examples[:num_shots]) + ("\n\n" if num_shots > 0 else "")
    cot_suffix = "\nThink step by step. " if td_prompt_style == "cot" else ""
    sr_suffix = "You are an AI assistant that uses a Chain of Thought (CoT) approach with reflection to answer queries. Follow these steps: \nStep 1. Reason through the problem step by step within the <reasoning> tags. \nStep 2. Given your previous reasoning, identify relevant temporal events in the given context for answering the given question within <timeline> tags. Assume relations in the context are unidirectional. \nStep 3. Reflect on your reasoning and the timeline to check for any errors or improvements within the <reflection> tags. \nStep 4. Make any necessary adjustments based on your reflection. If there is additional reasoning required, go back to Step 1 (reason through the problem step-by-step), otherwise move to Step 5. \nStep 5. Provide your final, concise answer within the <answer> tags. Provide the answer at the beginning of the response.  \nImportant: The <reasoning>, <timeline>, and <reflection> sections are for your internal reasoning process. Do not use enumerations or lists; use plain paragraph-style explanations. The final output must be entirely contained within the <answer> tags." if self_reflect else ""
        ...
        for line in f:
            ...
            for opt, label in zip(options, labels):
                if prompt_template ==1:
                    ...
                    base_prompt = (
                        f"{sr_suffix}"
                        f"Based on the dialogue:\n{context}\n"
                        f'Can the candidate word "{opt.strip()}" appropriately fill the <MASK> in this dialogue?\n'
                        f"{cot_suffix}The answer is:"
                    )
                elif prompt_template == 3:
                    base_prompt = (
                        f"{sr_suffix}"
                        f"According to the dialogue: \n{context}\n"
                        f'Is "{opt.strip()}" the appropriate word to fill in the <MASK> in this dialogue?\n'
                        f"{cot_suffix}The answer is:"
                    )
    ...
    for i in tqdm(range(0, len(data), batch_size), desc="Evaluating TimeDial"):
        batch = data[i:i + batch_size]

        if cot_suffix != "" or self_reflect:
            prompts = [d["prompt"] + cot_suffix for d in batch]
            ...
            for j, response in enumerate(trimmed_list):
                data[i + j]["score_yes"] = None  # not applicable for generation
                data[i + j]["score_no"] = None
                data[i + j]["pred"] = 1 if any(w in response.lower() for w in ["yes", "true", "correct"]) else 0
                # data[i + j]["pred"] = 1 if any(w in response.lower() for w in ["true"]) else 0
            ...
            last_logits = logits[:, -1, :]  # final token prediction
    ...
    print(f"[TimeDial] EM:{question_strict_acc:.4f}")
    print(f"[TimeDial] Question-level Marco F1:{question_f1:.4f}")
    print(f"[TimeDial] Option-level Micro F1:{option_micro_f1:.4f}")
    print(f"[TimeDial] Accuracy:{option_em:.4f}")
...
def evaluate_on_situatedgen(model, tokenizer, device, epoch, output_dir, config):
    ...
    prompts, ids = [], []
    for ex in dataset:
        base_prompt = f"Generate a pair of contrastive sentences with the given set of keywords: {ex['keywords']}. \nSentences: "
